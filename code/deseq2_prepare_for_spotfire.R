...
CNT <- fread(opt$cnt,colClasses="c")[, 1:4]  # featureCounts
...
#print(paste("start writing",sub("txt$|tsv$", "spotfire.txt", opt$de),"..."))
#print(dim(DE))
#fwrite(DE[, c("name", sort(names(DE)[2:(length(DE) - 1)]), "URL"), with=FALSE], 
#  file=sub("txt$|tsv$", "spotfire.txt.gz", opt$de), 
#  sep="\t", quote=FALSE, col.names=TRUE, nThread=1,
#  compress="gzip",buffMB=12)
#print(paste("end writing",sub("txt$|tsv$", "spotfire.txt", opt$de),"."))

# we write three tables, as they are too big otherwise
detab <- DE[, c("name", sort(names(DE)[2:(length(DE) - 1)]), "URL"), with=FALSE]
print(dim(detab))
b <- round(dim(detab)[2]/2)

print(paste("start writing first table ",sub("txt$|tsv$", "spotfire.1.txt", opt$de),"..."))
print(b)
fwrite(detab[, 1:b], 
  file=sub("txt$|tsv$", "spotfire.1.txt.gz", opt$de), 
  sep="\t", quote=FALSE, col.names=TRUE, nThread=1,
  compress="gzip")
print(paste("end writing first ",sub("txt$|tsv$", "spotfire.1.txt", opt$de),"."))

print(paste("start writing second table ",sub("txt$|tsv$", "spotfire.2.txt", opt$de),"..."))
detab2 <- detab[, (b+1):dim(detab)[2]]
print(dim(detab2))
fwrite(detab2, 
  file=sub("txt$|tsv$", "spotfire.2.txt.gz", opt$de), 
  sep="\t", quote=FALSE, col.names=TRUE, nThread=1,
  compress="gzip")
print(paste("end writing second",sub("txt$|tsv$", "spotfire.2.txt", opt$de),"."))

# dummy
print(paste("start writing third table ",sub("txt$|tsv$", "spotfire.3.txt", opt$de),"..."))
print(dim(detab2))
fwrite(detab2, 
  file=sub("txt$|tsv$", "spotfire.3.txt.gz", opt$de), 
  sep="\t", quote=FALSE, col.names=TRUE, nThread=1,
  compress="gzip")
print(paste("end writing third(3rd)",sub("txt$|tsv$", "spotfire.3.txt", opt$de),"."))

#print("start writing forth table ")
#print(dim(detab3))
#fwrite(detab3, 
#  file=sub("txt$|tsv$", "spotfire.4.txt.gz", opt$de), 
#  sep="\t", quote=FALSE, col.names=TRUE, nThread=1,
#  compress="gzip")
#print(paste("end writing third",sub("txt$|tsv$", "spotfire.4.txt", opt$de),"."))
