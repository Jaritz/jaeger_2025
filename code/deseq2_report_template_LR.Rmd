...
params:
  ...
  minCount: 100           # Minimum mean read count between the two conditions
  minTPM: 5              # Minimum mean TPM between the two conditions
  ...
  althyp: 'greaterAbs'   # Alternative hypothesis type
  fc4p: FALSE           # Include FC threshold in Ho.
---
...
# added 2023 April 13
# M Jaritz
fwrite(cbind(rownames(params$dds),vstmat),
      file=file.path(params$baseDir,
                          params$resultsDir,
                          paste(params$prefix, 'vsts.tsv', sep='.')),
...
# added 2023 April 13
# M Jaritz
fwrite(cbind(rownames(counts(params$dds)),tpm),
        file=file.path(params$baseDir,
                          params$resultsDir,
                          paste(params$prefix, 'tpms.tsv', sep='.')),
                         sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
cat(paste("Min required mean raw counts in either condition to accept a significant candidate:", params$minCount))
cat(paste("Min required mean TPM in either condition to accept a significant candidate:", params$minTPM))
...
```{r pca, fig.width=8, fig.width=8,eval=T}
for (x in names(cD)) {
...
```{r correlations1, fig.height=8, fig.width=8, eval=T}
...
```{r correlations2, fig.height=8, fig.width=10,eval=T}
...
```{r correlations3, fig.height=8, fig.width=8, eval=T}
...
```{r correlations4, fig.height=8, fig.width=10,eval=T}
...
if (is.null(params$reducedFormula)) {
  ...
  # Build model
  #print(paste("design obtained:",design(params$dds)))
        
  deobj <- DESeq(params$dds, test='LRT', reduced = as.formula(params$reducedFormula))
  
  #print(paste("design resulted:",deobj@design))
...
if (!is.null(params$reducedFormula)) {
  ...
  subres[, log2FoldChange := log2(FC)]
  #
  subres[log2FoldChange >= params$lfcthreshold, FCtext := '+']
  subres[log2FoldChange <= -params$lfcthreshold, FCtext := '-']
  subres[log2FoldChange < params$lfcthreshold & log2(FC) > -params$lfcthreshold, FCtext := '='] # unlikely
  subres[is.na(log2FoldChange), FCtext := '.']  # start time
  ...
  subres[, timecourse_raw := paste(FCtext, collapse=''), by='name']
  
  reslr <- merge(reslr, subres[, c('name', grp, 'FC', 'timecourse', 'timecourse_raw',  'FCtext'), with=FALSE], by=c('name', grp), all.x=TRUE)
...
```{r dexplr, fig.height=22, fig.width=10,eval=T}
if (!is.null(params$reducedFormula)) {
  # Counts
  matlr <- tpm[topreslr, ]
...
print(paste("DESeq design:",paste0(deobj@design,collapse = "")))
print(colData(deobj))

# added 2023 Aug 1
# M Jaritz
norm <- counts(deobj,normalized=TRUE)
fwrite(cbind(rownames(norm),norm),
      file=file.path(params$baseDir,
                    params$resultsDir,
                    paste(params$prefix, 'norm.tsv', sep='.')),
                    sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

## Function
do_de_plots <- function(contrast, dds = deobj, rpms = tpm, regcnt=vstmat, ntop = params$ntop, addPrefix = params$longlabel,
                        pcutoff = params$pcutoff, lfcthreshold = params$lfcthreshold, minC=params$minCount, minT=params$minTPM, 
                        filterBM=params$filterBaseMean, prefix=params$prefix, PMtype=colsuffix, rlog=params$rlog, fc4p=params$fc4p){
  # contrast <-  params$comparisons[[1]]
  # dds = deobj; rpms = tpm; ntop = params$ntop; addPrefix = params$longlabel; pcutoff = params$pcutoff; lfcthreshold = params$lfcthreshold; filterBM=params$filterBaseMean; prefix=params$prefix; PMtype=colsuffix; rlog=params$rlog; regcnt=vstmat; minC=params$minCount; minT=params$minTPM
  highlight <- c("non-significant", "significant, unmet thresh", "significant", paste("significant, top", params$ntop))
  ...
  lfc <- results(dds, test="Wald", contrast=contrast, 
                 independentFiltering = filterBM, alpha=pcutoff, lfcThreshold=ifelse(is.null(params$fc4p) || !params$fc4p, 0, params$lfcthreshold))
  ...
  cnt <- merge( data.table(name = rownames(counts(dds)),
                           ...
                           refMean = rowMeans(counts(dds)[, refs, drop=FALSE]),
                           treatMean = rowMeans(counts(dds)[, treats, drop=FALSE]),
                           maxCount = rowMaxs(counts(dds)[, c(refs, treats), drop=FALSE]),
                           refStDev = sqrt(rowVars(counts(dds)[, refs, drop=FALSE])),
                           treatStDev = sqrt(rowVars(counts(dds)[, treats, drop=FALSE])) ),
  ...
  outtab <- merge(lfc, 
                  shrink[, ! names(shrink) %in% c('baseMean', 'pvalue'), with=FALSE], 
                  by="name", all=TRUE, suffixes=c('', '.shrink') )
  ...
  # Thresholds.
  outtab[, meetthresh := !is.na(padj) & 
                        padj < pcutoff &
                        absl2fc >= lfcthreshold &
                        (refMean >= minC | treatMean >= minC) &
                        (refMeanTPM >= minT | treatMeanTPM >= minT)]
  ...
  ppen <- max(outtab$mlog10p[is.finite(outtab$mlog10p)]) / max(outtab$absl2fc[is.finite(outtab$absl2fc)])  # scaling factor determined by axes ranges
  ...
  if (nrow(outtab[(meetthresh), ]) == 0)
    outtab[, istop := padj < pcutoff]    # Out of principle, no highlight labels for non-sig features.
  ...
  zerosb <- min(outtab[refMean > 0, refMean], outtab[treatMean > 0, treatMean]) / 10
  
  ## Volcanos
  #print(head(outtab))
  #print(head(regcnt))
  
  # this is printed every time for each comparison - should be optimized at some point!
  #print(str(regcnt))
  ggp <- GGally::ggpairs(data.frame(regcnt),title = ifelse(params$rlog, "rlog_count", "VST_count"))
  ...
  pv1 <- ggplot(outtab, 
            ...
            geom_point(aes(size=refMeanTPM), alpha=0.5) +
            geom_text_repel(data=outtab[(istop), ], size=rel(2.5),  colour="violet",
                            segment.size = 0.3, min.segment.length=0, max.overlaps=1e4, force=1, force_pull=1) +
  ...
  pma1 <- ggplot(NULL, 
            ...
            geom_hline(yintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
            geom_text_repel(data=outtab[(istop), ], colour="violet", size=rel(2.5), segment.size = 0.3, min.segment.length=0, max.overlaps=1e4, force=1, force_pull=1) +
            scale_x_log10() +
  ...
  pma2 <- ggplot(NULL, 
            ...
            geom_hline(yintercept = lfcthreshold, linetype="dashed",  colour="steelblue1") + 
            geom_hline(yintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
            geom_text_repel(data=outtab[name %in% subfc[(istop), name],], colour="violet", size=rel(2.5), segment.size = 0.3, min.segment.length=0, max.overlaps=1e4, force=1, force_pull=1) +
            scale_x_log10() +
            annotation_logticks(base=10, sides='b') +
  ...
  if (is.null(dim(selcnt))) {
    ...
    selvst <- as.data.table( c(list(name=subfc$name), as.list(selvst[c(refs, treats)])) )
    selvst$type <- ifelse(rlog, "rlog_count", "VST_count")
    selcnt <- as.data.table( c(list(name=subfc$name), as.list(selcnt[c(refs, treats)])) )
    selcnt$type <- PMtype
  ...
  pe1 <- ggplot(NULL, aes(x=refMeanTPM + zeros, y=treatMeanTPM + zeros, label=name, colour=category)) +
      ...
      geom_label_repel(data=outtab[category == highlight[4], ], size=rel(2.5), colour="violet",
  ...
  pe1b <- ggplot(NULL, aes(x=refMean + zerosb, y=treatMean + zerosb, label=name, colour=category)) +
      geom_abline(intercept = 0, slope = 1, colour='black') +
      geom_point(data=outtab[category == highlight[1], ], alpha=0.4, size=rel(0.8)) +
      geom_point(data=outtab[category == highlight[2], ], alpha=0.7, size=rel(0.8)) +
      geom_point(data=outtab[category == highlight[3], ], alpha=0.7) +
      ...
      scale_colour_manual(values=hlcol) +
      annotation_logticks(base=10, sides='lrtb') +
      labs(title=ctrname, x='reference Mean Reads', y='treatment Mean Reads') +
      theme(aspect.ratio=1, panel.grid.minor = element_blank())
  ...
  pe4 <- ggplot(NULL, aes(y=refMeanTPM + zeros, x=log2FoldChange, label=name, colour=category)) +
      geom_vline(xintercept = lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_vline(xintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_point(data=outtab[category == highlight[1], ], alpha=0.4, size=rel(0.8),  colour="black") +
      geom_point(data=outtab[category == highlight[2], ], alpha=0.7, size=rel(0.8),  colour="black") +
      geom_point(data=outtab[category == highlight[3], ], alpha=0.7,  colour="black") +
      geom_point(data=outtab[category == highlight[4], ]) +
      geom_label_repel(data=outtab[category == highlight[4], ], size=rel(2.5), colour="violet",
                       segment.size = 0.3, min.segment.length=0, max.overlaps=1e4, force=10, force_pull=5) +
      scale_y_log10() + 
      ...
      labs(title=ctrname, y=paste('reference Mean', PMtype), x='log2( FC )') +
      theme(aspect.ratio=1, legend.position='none',  panel.grid.minor = element_blank())
  
    pe4b <- ggplot(NULL, aes(y=refMean + zerosb, x=log2FoldChange, label=name, colour=category)) +
      geom_vline(xintercept = lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_vline(xintercept = -lfcthreshold, linetype="dashed",  colour="steelblue1") + 
      geom_point(data=outtab[category == highlight[1], ], alpha=0.4, size=rel(0.8),  colour="black") +
      geom_point(data=outtab[category == highlight[2], ], alpha=0.7, size=rel(0.8),  colour="black") +
      ...
      scale_colour_manual(values=hlcol) +
      annotation_logticks(base=10, sides='lr') +
      labs(title=ctrname, y='reference Mean Reads', x='log2( FC )') +
      theme(aspect.ratio=1, legend.position='none',  panel.grid.minor = element_blank())
  ## Table pt2
  ...
  custom <- NULL
  if (addPrefix) {
    # Keep a pristine copy, without prefixes.
    custom <- copy(outtab)
    
    # Drop columns I don't need
    keep <- c("name", "baseMean", "FC", "log2FoldChange", "log2FoldChange.shrink", "pvalue", "padj",  "mlog10p", 
              "refMean", "treatMean", "maxCount", "refMeanTPM", "treatMeanTPM", "maxScaledCount")
    custom <- custom[, keep, with=FALSE]
    ...
    setnames(custom, c(names(custom)[1],      # keep uniform name across tables for use as joining key
                       paste(names(custom)[2:length(custom)],  prefix, sep='.')) )
  ...
  return(list(contrast=ctrname, 
              ...
              pe1=pe1, pe1b=pe1b, pe2=pe2, pe3=pe3, pe4=pe4, pe4b=pe4b, #pe1ly=ggplotly(pe1ly, dynamicTicks = TRUE),
              ph1=ph1, ph2=ph2,ggp=ggp, 
              tophits=rn, hashits=hashits,
              outtab=outtab[, c("name", "baseMean", "log2FoldChange", "lfcSE", "stat", "pvalue", "padj",
                                "log2FoldChange.shrink", "lfcSE.shrink", "padj.shrink"), with=FALSE], 
              custom=custom))
}
##
...
```{r expov, warning=FALSE, fig.height=13.5, fig.width=15,eval=T}
for (res in resall) {
  # res <- resall[[1]]
...
```{r csv}
if (!is.null(params$reducedFormula)) {
  ofile <- file.path(params$baseDir,
                          params$resultsDir,
...
if (!is.null(params$comparisons)) {
  ...
  for (res in resall) {
    ...
    if (params$longlabel)
      ofile <- file.path(params$baseDir,
                            params$resultsDir,
                            paste(params$prefix, res$contrast, 'custom.deseq2.tsv', sep='.'))
      print(ofile)
      fwrite(res$custom,
           file = ofile,
           sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
# get plot
get_fc_plot <- function(allhitdf,col_title,rn) {
    
  #print("get_fc_plot:")  
  #print(head(allhitdf))
  
  # for color scale
  # make sure that 0 is included in scale
  max <- round(max(abs(na.omit(allhitdf[,-"Feature"]))))
  #print("max:")
  #print(max)
    
  # just in case, check if scale is possible:
  breaks.try <- try(
    breaks <- sort(unique(c(seq.int(-max,max,by=round(max/4)),0)))
  )
  if(class(breaks.try) == "try-error") {
    breaks <- NULL
  }

  #return(NULL)
  
  allhitdf <- melt(allhitdf, 
                   value.name = col_title, 
                   variable.name = "Sample", 
                   id.vars = "Feature")
  allhitdf[, Feature := ordered(Feature, levels=rn)]
  #print(head(allhitdf))
  #print(summary(allhitdf))
  
  palllog2fc <- ggplot(allhitdf, 
                       aes_string(y="Feature", x="Sample", fill=col_title)) +
    geom_tile() +
    scale_fill_gradient2(high="darkgreen", low="darkred", mid="white",na.value='gray50',
                         breaks=breaks) +
    coord_equal(ratio=1) +
    labs(y='', x='') +
    ggtitle(col_title) +
...
if (!is.null(params$comparisons)) {
  ...
  #
  # Log2FC | log2FC shrink
  #
  #print("LOG2FC:")
  rfile1 <- str_remove(params$selfname,".deseq2.html")
  
  i <- 1
  if(exists("log2fc.df")) { remove("log2fc.df") }
  if(exists("log2fc")) { remove("log2fc") }
  if(exists("log2fcs.df")) { remove("log2fcs.df") }
  if(exists("log2fcs")) { remove("log2fcs") }
  ...
  for (comp in params$comparisons) {
    ...
    #print(comp)
    # DeSeq2 output file
    comp.name <- paste0(comp[1],"_",comp[2],"_vs_",comp[3])
    #print(comp.name)
    #print(params$baseDir)
    #print(params$resultsDir)
    rfile2 <- paste0(params$baseDir,"/",params$resultsDir,"/",rfile1,".",comp.name,".deseq2.tsv")
    #print(rfile2)
    rfile.t <- read_delim(rfile2,delim="\t",col_names=TRUE)
    
    log2fc <- rfile.t$log2FoldChange
    log2fcs <- rfile.t$log2FoldChange.shrink
    #print(head(log2fc))
    
    if(!exists("log2fc.df")) {
      log2fc.df <- data.frame(log2fc)
      rownames(log2fc.df) <- rfile.t$name
      colnames(log2fc.df) <- comp.name
    } else {
      log2fc.df <- cbind(log2fc.df,log2fc)
      colnames(log2fc.df)[i]  <- comp.name
    }
    
    if(!exists("log2fcs.df")) {
      log2fcs.df <- data.frame(log2fcs)
      rownames(log2fcs.df) <- rfile.t$name
      colnames(log2fcs.df) <- comp.name
    } else {
      log2fcs.df <- cbind(log2fcs.df,log2fcs)
      colnames(log2fcs.df)[i]  <- comp.name
    }
  ...
  #print("log2fc.df")
  #print(head(log2fc.df))
  #print(dim(log2fc.df))
  #print("mat3")
  
  log2fc <- log2fc.df[allhits,]
  #print(head(mat3))
  #print(rownames(mat3))
  
  allhitdf <- as.data.table(log2fc)
  allhitdf[, Feature := allhits]
  #print("allhitdf:")
  #print(head(allhitdf))
  
  palllog2fc  <- get_fc_plot(allhitdf,"log2fc",rn)
  
  #print(pallvst + palllog2fc)
  
  # shrunken fold change:
  #print(head(log2fcs.df))
  #print(dim(log2fcs.df))
  log2fcs <- log2fcs.df[allhits,]
  allhitdf <- as.data.table(log2fcs)
  allhitdf[, Feature := allhits]
  
  palllog2fcs <- get_fc_plot(allhitdf,"log2fcs",rn)
  
  print(pallvst + palllog2fc + palllog2fcs)
